...
export default class RunebasePredictionStore {
  ...
  constructor(app) {
    ...
    reaction(
      ...
      () => {
        if (this.app.ui.location === Routes.RUNEBASE_PREDICTION) {
          this.init();
        }
      }
    );
    reaction(
      ...
      () => {
        if (this.app.ui.location === Routes.RUNEBASE_PREDICTION && this.app.global.online) {
  ...
  init = async () => {
    ...
    this.app.ui.location = Routes.RUNEBASE_PREDICTION;
  ...
  async fetch(limit = this.limit, skip = this.skip) {
    if (this.hasMore) {
      ...
      const filters = [
        { token: Token.RUNEBASE, status: OracleStatus.VOTING, language: this.app.ui.locale },
        { token: Token.RUNEBASE, status: OracleStatus.CREATED, language: this.app.ui.locale },
      ];
