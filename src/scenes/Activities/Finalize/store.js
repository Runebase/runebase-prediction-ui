...
export default class {
  ...
  constructor(app) {
    ...
    reaction(
      ...
      () => {
        if (this.app.ui.location === Routes.FINALIZE && this.app.global.online) {
  ...
  fetch = async (limit = this.limit, skip = this.skip) => {
    ...
    // we want to fetch all *Oracles* which is related to PRED token and waitResult status
    if (this.hasMore) {
      const filters = [{ token: Token.PRED, status: OracleStatus.WAIT_RESULT, language: this.app.ui.locale }];
