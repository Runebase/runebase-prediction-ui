...
export default class EventStore {
  ...
  @observable allowance = INIT.allowance; // In Predoshi
  @observable activeStep = INIT.activeStep;
  @observable error = INIT.error

  // topic
  @observable topics = INIT.topics
  @observable runebaseWinnings = INIT.runebaseWinnings
  @observable predWinnings = INIT.predWinnings
  ...
  @computed get cOracle() {
    // [RUNEBASE] - CREATED -> BETTING -> WAITRESULT -> OPENRESULTSET -> PENDING -> WITHDRAW
    // mainly: BETTING & RESULT SETTING phases
    return find(this.oracles, { token: Token.RUNEBASE }) || {};
  }
  @computed get oracle() {
    if (this.unconfirmed) {
  ...
  initTopic = async () => {
    ...
    await this.queryLeaderboard(Token.RUNEBASE);
   ...
   * The allowance is the current amount of PRED approved for transfer.
   */
  @action
  ...
  calculateWinnings = async () => {
    ...
    this.predWinnings = sumBy(this.withdrawableAddresses, a => (
      a.type === TransactionType.WITHDRAW && a.predWon ? a.predWon : 0
    ));
    this.runebaseWinnings = sumBy(this.withdrawableAddresses, ({ runebaseWon }) => runebaseWon);
  }
  ...
  getWithdrawableAddresses = async () => {
    ...
    try {
      ...
      for (let i = 0; i < filtered.length; i++) {
        ...
        const predWon = data ? satoshiToDecimal(data[0]) : 0;
        const runebaseWon = data ? satoshiToDecimal(data[1]) : 0;
  ...
  disableEventActionsIfNecessary = () => {
    ...
    const currentWalletRunebase = wallet.currentWalletAddress ? wallet.currentWalletAddress.runebase : 0;
    ...
    // Trying to set result or vote when not enough RUNEBASE or PRED
    ...
    if ((
      ...
      this.buttonDisabled = true;
      this.error.amount = 'str.notEnoughRunebaseAndPred';
      return;
    }
    ...
    // Trying to bet more runebase than you have or you just don't have enough RUNEBASE period
    if ((phase === BETTING && this.amount > currentWalletRunebase + maxTransactionFee) || notEnoughRunebase) {
      this.buttonDisabled = true;
      this.error.amount = 'str.notEnoughRunebase';
      return;
    }

    // Not enough pred for setting the result or voting
    if ((phase === RESULT_SETTING && currentPred < consensusThreshold && this.selectedOptionIdx !== -1)
      || (phase === VOTING && currentPred < this.amount)) {
      this.buttonDisabled = true;
      this.error.amount = 'str.notEnoughPred';
      return;
    }
