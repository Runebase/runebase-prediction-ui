...
export default class PredCourtStore {
  ...
  constructor(app) {
    ...
    reaction(
      ...
      () => {
        if (this.app.ui.location === Routes.PRED_COURT) {
          this.init();
        }
      }
    );
    reaction(
      ...
      () => {
        if (this.app.ui.location === Routes.PRED_COURT && this.app.global.online) {
  ...
  init = async (limit = this.limit) => {
    ...
    this.app.ui.location = Routes.PRED_COURT;
  ...
  async fetch(limit = this.limit, skip = this.skip) {
    if (this.hasMore) {
      ...
      const filters = [
        { token: Token.PRED, status: OracleStatus.VOTING, language: this.app.ui.locale },
