...
const messages = defineMessages({
  ...
  strNotEnoughPredMsg: {
    id: 'str.notEnoughPred',
    defaultMessage: "You don't have enough PRED",
  },
  createRequiredMsg: {
    id: 'create.required',
...
export default class CreateEventStore {
  ...
  @computed get hasEnoughFee() {
    ...
    return currentWalletAddress
      && (currentWalletAddress.runebase >= transactionFee)
      && (currentWalletAddress.pred >= this.escrowAmount);
  }
  @computed get warning() {
    if (!this.hasEnoughFee) {
      return {
        id: 'str.notEnoughRunebaseAndPred',
        message: 'You don\'t have enough RUNEBASE or PRED',
      };
    }
    return {};
  }
  ...
  open = async () => {
    ...
    if (isEmpty(this.app.wallet.currentAddress)) {
      this.app.runebasechrome.openPopover('runebasechrome.loginToView');
      this.close();
      return;
    }
  ...
  validateCreator = () => {
    ...
    if (checkingAddresses.length && checkingAddresses[0].pred < escrowAmount) {
      this.error.creator = messages.strNotEnoughPredMsg.id;
    } else if (!creator) {
