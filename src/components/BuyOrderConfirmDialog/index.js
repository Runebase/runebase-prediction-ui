...
@inject('store')
@observer
export default class BuyOrderConfirmDialog extends Component {
  render() {
    ...
    let confirmMessage = formatMessage(messages.orderConfirmMessageMsg, { txDesc, txAmount, txToken });
    if (txFee) {
      confirmMessage = formatMessage(messages.orderConfirmMessageWithFeeMsg, { txDesc, txAmount, txToken, txFee });
    }
    if (!open) {
      return null;
    }
    ...
    return (
      <Dialog open={open}>
        <DialogTitle>
          <FormattedMessage id="orderConfirm.title" defaultMessage="Please Confirm Your Order" />
        </DialogTitle>
        <DialogContent>
          {confirmMessage}
