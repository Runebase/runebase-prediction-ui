...
const messages = defineMessages({
  cancelOrderConfirmMessageMsg: {
    id: 'cancelOrderConfirm.message',
    defaultMessage: 'You are about to {txDesc} for {txAmount} {txToken}. Please click OK to continue.',
  },
  cancelOrderConfirmMessageWithFeeMsg: {
    id: 'cancelOrderConfirm.messageWithFee',
...
@inject('store')
@observer
export default class CancelOrderConfirmDialog extends Component {
  render() {
    ...
    let confirmMessage = formatMessage(messages.cancelOrderConfirmMessageMsg, { txDesc, txAmount, txToken });
    if (txFee) {
      confirmMessage = formatMessage(messages.cancelOrderConfirmMessageWithFeeMsg, { txDesc, txAmount, txToken, txFee });
    }
    if (!open) {
      return null;
    }
    ...
    return (
      <Dialog open={open}>
        <DialogTitle>
          <FormattedMessage id="cancelOrderConfirm.title" defaultMessage="Please Confirm Your Transaction" />
        </DialogTitle>
        <DialogContent>
          {confirmMessage}
