...
export default class GlobalStore {
  ...
  @observable rweb3 = INIT_VALUES.rweb3
    ...
    reaction(
      ...
      () => {
        if (this.localWallet === false) {
          this.app.wallet.fetchPredBalance(this.app.wallet.currentAddress);
        }
      }
    );

    // Set flag of using a local wallet, eg. Runebase Wallet vs RunebaseChrome
  ...
  onSyncInfo = (syncInfo) => {
    if (syncInfo.error) {
      ...
      // Only use the syncInfo balances if using a local wallet. RunebaseChrome will set the addresses differently.
      if (this.localWallet) {
  ...
  getActionableItemCount = async () => {
    ...
    try {
      ...
      const oracleSetFilters = [{ token: Token.RUNEBASE, status: OracleStatus.OPEN_RESULT_SET, language: this.app.ui.locale }];
      ...
      // Get finalize items
      const oracleFinalizeFilters = [{ token: Token.PRED, status: OracleStatus.WAIT_RESULT, language: this.app.ui.locale }];
