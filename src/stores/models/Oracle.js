...
export default class Oracle {
  ...
  token = '' // PRED or RUNEBASE
  amounts = [] // RUNEBASE or PRED amounts array for each result index
  ...
  resultSetterAddress = '' // Result setter's Runebase address.
  ...
  @computed get phase() {
    ...
    if (RUNEBASE && ['PENDING', 'WITHDRAW', 'CREATED', 'VOTING'].includes(status)) return BETTING;
    if (PRED && ['PENDING', 'VOTING', 'WITHDRAW'].includes(status)) return VOTING;
    if (RUNEBASE && ['WAITRESULT', 'OPENRESULTSET'].includes(status)) return RESULT_SETTING;
  ...
  @computed get isOpenResultSetting() {
    return this.token === Token.RUNEBASE && this.status === 'OPENRESULTSET';
  }
  // Archived Oracles mean their purpose has been served. eg. Betting Oracle finished betting round.
  @computed get isArchived() {
    const { token, status } = this;
    const [PRED, RUNEBASE] = [token === Token.PRED, token === Token.RUNEBASE];
    if (RUNEBASE && ['PENDING', 'WITHDRAW'].includes(status)) return true; // BETTING
  ...
  @computed get isPending() {
    ...
    const [RUNEBASE] = [token === Token.RUNEBASE];
