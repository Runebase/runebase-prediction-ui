...
const INIT_VALUES = {
  visible: false,
  provider: WalletProvider.RUNEBASECHROME,
};
...
export default class TransactionStore {
  ...
  constructor(app) {
    ...
    // RunebaseChrome logged in/out
    reaction(
      () => this.app.runebasechrome.loggedIn,
      () => {
        if (this.app.runebasechrome.loggedIn) {
          this.checkPendingApproves();
        } else {
          Object.assign(this, INIT_VALUES);
  ...
  executeApprove = async (senderAddress, spender, amountSatoshi) => {
    const runebasepredictionToken = getContracts().RunebasePredictionToken;
    const contract = this.app.global.rweb3.Contract(runebasepredictionToken.address, runebasepredictionToken.abi);
   ...
   * @param {string} resultSettingStartTime Unix timestamp of the result setting start time.
   * @param {string} resultSettingEndTime Unix timestamp of the result setting end time.
   * @param {string} amount Escrow amount in Predoshi.
   */
  @action
  ...
  executeCreateEvent = async (index, tx) => {
    try {
      ...
      const contract = this.app.global.rweb3.Contract(
      ...
      if (txid) {
        ...
        await this.onTxExecuted(tx);
        this.app.runebasePrediction.loadFirst();
  ...
  executeBet = async (index, tx) => {
    try {
      ...
      const contract = this.app.global.rweb3.Contract(oracleAddress, getContracts().CentralizedOracle.abi);
  ...
  executeVote = async (index, tx) => {
    try {
      ...
      const contract = this.app.global.rweb3.Contract(oracleAddress, getContracts().DecentralizedOracle.abi);
  ...
  executeWithdraw = async (index, tx) => {
    try {
      ...
      const contract = this.app.global.rweb3.Contract(topicAddress, getContracts().TopicEvent.abi);
