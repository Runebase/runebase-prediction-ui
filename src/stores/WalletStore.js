...
export default class WalletStore {
  ...
  @computed get lastAddressWithdrawLimit() {
    return {
      RUNEBASE: this.currentWalletAddress ? this.currentWalletAddress.runebase : 0,
      PRED: this.currentWalletAddress ? this.currentWalletAddress.pred : 0,
    };
  }
  ...
  /**
   * Sets the account sent from RunebaseChrome.
   * @param {object} account Account object.
   */
  @action
  onRunebaseChromeAccountChange = async (account) => {
    ...
    if (network.toLowerCase() !== process.env.CHAIN_NETWORK) {
      this.app.runebasechrome.openPopover('runebasechrome.loggedIntoWrongNetwork');
      return;
    }

    // If setting RunebaseChrome's account for the first time or the address changes, fetch the PRED balance right away.
    // After the initial PRED balance fetch, it will refetch on every new block.
    const fetchInitPredBalance = isEmpty(this.addresses);

    const index = findIndex(this.addresses, { address });
    if (index === -1) {
      // Push the WalletAddress if it is not in the list of addresses
      const walletAddress = new WalletAddress({ address, runebase: balance, pred: 0 }, false);
      this.addresses.push(walletAddress);
      this.currentWalletAddress = walletAddress;
    } else {
      // Update existing balances
      const walletAddress = this.addresses[index];
      walletAddress.runebase = balance;
      if (this.currentWalletAddress.address === address) {
        this.currentWalletAddress.runebase = balance;
      }
    }

    if (fetchInitPredBalance) {
      this.fetchPredBalance(address);
    }
  }

  /**
   * Checks the RunebasePrediction Token allowance from the owner to spender.
  ...
  /**
   * Calls the RunebasePredictionToken contract to get the PRED balance and sets the balance in the addresses.
   * @param {string} address Address to check the PRED balance of.
   */
  fetchPredBalance = async (address) => {
    if (isEmpty(address)) {
      return;
    }

    try {
      const { data } = await axios.post(Routes.api.predBalance, {
        owner: address,
        senderAddress: address,
      });
      if (data.balance) {
        const pred = satoshiToDecimal(data.balance);

        // Update WalletAddress PRED in list of addresses
        const index = findIndex(this.addresses, { address });
        if (index !== -1) {
          this.addresses[index].pred = pred;
        }

        // Update current WalletAddress PRED if matching address
        if (this.currentWalletAddress && this.currentWalletAddress.address === address) {
          this.currentWalletAddress.pred = pred;
        }
      }
    } catch (err) {
      console.error(`Error getting PRED balance for ${address}: ${err.message}`); // eslint-disable-line
    }
  }
  ...
  withdraw = async (walletAddress) => {
    ...
    const amount = this.selectedToken === Token.PRED
