...
/**
 * Converts the gas number to RUNEBASE cost.
 * @param gas {Number} The gas number to convert.
 * @return {Number} The gas amount represented as RUNEBASE.
 */
...
export const getPhase = ({ token, status }) => {
  const [PRED, RUNEBASE] = [token === 'PRED', token === 'RUNEBASE'];
  if (RUNEBASE && status === 'CREATED') return UNCONFIRMED; // BETTING
  if (RUNEBASE && status === 'VOTING') return BETTING;
  if (PRED && status === 'VOTING') return VOTING;
  if (RUNEBASE && ['WAITRESULT', 'OPENRESULTSET'].includes(status)) return RESULT_SETTING;
  if ((PRED || RUNEBASE) && status === 'PENDING') return PENDING; // VOTING
  if (PRED && status === 'WAITRESULT') return FINALIZING;
  if ((PRED || RUNEBASE) && status === 'WITHDRAW') return WITHDRAWING;
