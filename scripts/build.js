...
process.argv.forEach((arg) => {
  if (arg.startsWith('--chain=')) {
    ...
    switch (chain) {
      case 'mainnet': {
        hostname = 'pred.runebase.io';
        apiPort = 8989;
        break;
      }
      case 'testnet': {
        hostname = 'testpred.runebase.io';
        apiPort = 6767;
        break;
      }
      case 'regtest': {
        hostname = 'regpred.runebase.io';
        apiPort = 5555;
        break;
      }
      default: {
...
// The localwallet flag lets the UI know that it is using a local wallet (Runebase wallet) to handle transactions.
// Default logic is to use RunebaseChrome for transactions.
