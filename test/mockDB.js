...
export default {
  ...
  generateTopic(params) {
    const topic = {
      ...
      options: ['A', 'B', 'C'],
      runebaseAmount: times(3, () => decimalToSatoshi(randomInt(1, 10))),
  ...
  generateWithdraw(params) {
    const isRunebase = randomInt(0, 1);
    const tx = new Withdraw({
      ...
      runebaseAmount: isRunebase ? decimalToSatoshi(randomInt(1, 10)) : 0,
      botAmount: !isRunebase ? decimalToSatoshi(randomInt(1, 10)) : 0,
      version: 0,
      type: !isRunebase ? 'ESCROW' : 'WINNING',
    });
    Object.assign(tx, params);
    return tx;
  },
  ...
  generateVote(params) {
    ...
    const tx = new Vote({
      ...
      version: 0,
      token: whichType === 0 ? Token.RUNEBASE : Token.BOT,
